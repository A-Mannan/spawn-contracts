...
A flush — single pool or batch — snapshots and zeros the complete new pot of every pool it touches before any interaction, enters the global payout guard once, and — only when some pot is non-zero — opens one `PoolManager.unlock` with a dedicated `REDEEM_PAYOUT_POT` action carrying the per-pot amounts. The callback redeems precisely that claim total to the hook in one burn and one take, and the outer frame emits one `PayoutPotRedeemed` event per pot, so attribution stays per-pool while the unwrap is shared. It cannot use a generic claim top-up path, because whole-pot, once-only redemption must be independently auditable and must not consume claims backing protocol revenue. Carry-only flushes perform no unlock.

**Why:** PoolManager claims solve the in-flight swap shortfall without relying on unrelated singleton liquidity. The claim minted by a harvest backs both the payout pot and its service fee until each liability class is redeemed. A dedicated pot redemption action proves that the tip base and new allocation base correspond to fully redeemed pots and are not mixed with direct or protocol claims; sharing one redemption across a batch is pure gas arithmetic — the events, the zeroing order, and the per-pot amounts in the unlock payload preserve the audit trail.

**Alternatives considered:** Redeem during `afterSwap` (can fail before the swapper settles); reuse `_ensureEth` (may redeem only a shortfall and commingle liability classes); let each plugin redeem its share (multiple unlocks and plugin authority over custody); one unlock per pool inside a batch (saves nothing over a shared burn and forfeits the batching gain).
...
The directed tip recipient receives the batch's combined tip through one ordinary ETH transfer before plugin iteration — the tip itself is still computed per pot (`floor(newPot / 100)`) because each pool's allocation base is its own post-tip remainder, but the transfers collapse into one. There is no implicit caller default: `flushTo(poolId, tipTo)` flushes a single pool and `flushBatch(pools, tipTo)` flushes many; a caller directing the tip to itself passes its own address. If that transfer fails, the flush reverts atomically: pot zeroing/redemption, carry changes, plugin calls, events, and every transfer in the flush are rolled back — and in a batch, every pool in the batch with it, which is the specified all-or-nothing semantics (callers wanting per-pool isolation use multicall over single-pool flushes). The hook also exposes `flushGasCeiling(poolId)` and `creatorPathGasCeiling(poolId)` — worst-case outer gas built from the registered stipends and the published constants — so batch callers can size a transaction against the block gas budget; a batch's ceiling is the sum of its pools' ceilings, which over-reserves by the shared redemption and is therefore safe. The payout path then iterates each pool's plan's at-most-eight set bits in ascending order. For each entry, `attempted = pluginShare_i + previousCarry_i`; carry is cleared before the external call. Plugin success is exactly the EVM `CALL` success bit: a non-reverting `void onPayout(PoolId,address)` call succeeds, and the core ignores all returndata, whether empty, malformed, or arbitrarily long. A zero success bit—including explicit revert or stipend exhaustion—restores the full attempted value as carry. A suspended or codehash-mismatched entry is not called: its attempted amount is credited to creator entitlement and its carry is permanently cleared. A zero attempted amount is skipped.

After each pool's plugin iteration, `creatorNew` plus all redirects is credited—not pushed—to `_creatorPathClaimable[poolId]`. `claimCreatorPath` is likewise available as a batch: `claimCreatorPathBatch(pools)` authenticates the caller as every pool's current holder, shares the batch redemption, rechecks ownership after every plugin interaction (any change reverts the complete batch), and then attempts each pool's complete entitlement including its retained tip with the same restore-on-failure semantics — a failed final transfer restores only that pool's entitlement and does not revert the batch. A carry-only flush iterates `_carryBitmap` with no redemption, allocation, or tip. Empty pot plus empty bitmap returns before any unlock or transfer.
