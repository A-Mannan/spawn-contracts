...
import {Phase} from "../../src/types/LaunchTypes.sol";
...
contract MilestoneHookPermissionsTest is Test {
    /// @dev Low 14 bits must equal the required flag mask (15040 == 0x3AC0). High bits are arbitrary;
    /// this stands in for a mined address.
    ...
    function setUp() public {
        ...
        coldPaths = new MilestoneColdPaths(IPoolManager(POOL_MANAGER), nft, support);

        deployCodeTo("MilestoneHook.sol:MilestoneHook", _ctorArgs(), HOOK_ADDR);
    ...
    function _ctorArgs() internal view returns (bytes memory) {
        return abi.encode(IPoolManager(POOL_MANAGER), nft, support, coldPaths, PROTOCOL_ADMIN, PROTOCOL_RECIPIENT);
    }
    ...
    // --- Declared permissions match the spec's flag set ---
    ...
    function test_constructorWiring() public view {
        assertEq(address(hook.poolManager()), POOL_MANAGER, "pool manager");
    ...
    function test_constructorRejectsZeroAdmin() public {
        (bool ok, bytes memory ret) = _tryConstruct(
            abi.encode(IPoolManager(POOL_MANAGER), nft, support, coldPaths, address(0), PROTOCOL_RECIPIENT)
        );
    ...
    function test_constructorRejectsZeroRecipient() public {
        (bool ok, bytes memory ret) =
            _tryConstruct(abi.encode(IPoolManager(POOL_MANAGER), nft, support, coldPaths, PROTOCOL_ADMIN, address(0)));
    ...
    function test_constructorRejectsCodelessColdPaths() public {
        (bool ok, bytes memory ret) = _tryConstruct(
            abi.encode(IPoolManager(POOL_MANAGER), nft, support, address(0xC0DE), PROTOCOL_ADMIN, PROTOCOL_RECIPIENT)
        );
    ...
    function test_unknownPoolIsPhaseNone() public view {
        ...
        assertEq(hook.curveCount(unknown), 0, "no curves");
        assertFalse(hook.liveBand(unknown).deployed, "no live band");
    }
    ...
    function test_coldPathsRejectDirectLaunch() public {
        MilestoneBase.LaunchParams memory params;

        vm.expectRevert(MilestoneColdPaths.NotDelegated.selector);
        coldPaths.launch(params);
    }
    ...
    function test_hookAcceptsNativeEth() public {
        ...
        assertEq(address(hook).balance, 1 ether, "balance credited");
    }
}
