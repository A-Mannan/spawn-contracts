...
- Add reference plugins for buyback-and-burn and creator payout, plus an opted-in same-transaction swap-and-flush helper. Creator plugin payout flushes first and returns the tip to the actual flusher. An LP-compounding payout plugin is excluded from this change.
