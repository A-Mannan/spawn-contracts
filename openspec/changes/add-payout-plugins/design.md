...
- Preserve solvency across raw ETH, PoolManager claims, direct creator revenue, global protocol revenue, payout pots, creator-plugin entitlement, and failed plugin carry.
...
**Decision:** Retain `MilestoneHook` for callbacks and bounded harvest accounting, retain `MilestoneColdPaths` for launch, graduation, fee collection, and existing lifecycle unlocks, and add `MilestonePayoutPaths` for flush, plugin delivery, carry retry, and creator-plugin claiming. All three inherit `MilestoneBase`; neither derived hook nor either satellite declares mutable state. The hook exposes thin wrappers that delegate to the appropriate immutable satellite.
...
Entry registration rejects zero or duplicate addresses, contracts without code, invalid roles, zero/unsafe gas limits, takes above one whole, and proxy-like or otherwise unpinnable entries. Runtime delivery requires the registered `extcodehash`; a mismatch is handled like suspension and redirects current share plus carry to the creator rather than invoking changed code. Creator-sink and utility roles are not selectable plan bits.
...
Canonical deployment registers buyback-and-burn at a published stable index with `takeWad = floor(2 * WAD / 9)` and publishes the one-bit default plan. The creator receives the exact arithmetic remainder, so WAD representability and all division dust favor the mandatory creator sink.
...
**Decision:** `_harvestBand` continues to burn the completed position and mint a quote ERC-6909 claim. It records gross attribution, calculates the service fee from one snapshot, credits the global protocol ledger, and credits the exact remainder to `_payoutPot[poolId]`. It performs no destination call, buyback, donation, or real-ETH transfer.

A flush snapshots and zeros the complete new pot before interaction, enters the global payout guard, and—only when the new pot is non-zero—opens one `PoolManager.unlock` with a dedicated `REDEEM_PAYOUT_POT` action containing pool and exact amount. The callback redeems precisely that claim quantity to the hook. It cannot use the generic claim top-up path, because whole-pot, once-only redemption must be independently auditable. Carry-only flushes perform no unlock.

**Why:** PoolManager claims solve the in-flight swap shortfall without relying on unrelated singleton liquidity. A dedicated redemption action proves that the tip base and new allocation base correspond to one fully redeemed pot and are not mixed with direct or protocol claims.
...
mapping(PoolId => uint256) internal _creatorPluginClaimable;
mapping(PoolId => uint256) internal _creatorClaimable;
uint256 internal _protocolClaimable;
```

Aggregate totals for pots, carry, creator-plugin balances, and direct/global claims are maintained when useful for solvency assertions and invariant observability. Raw ETH plus redeemable quote claims must cover all liabilities; no claim function treats the contract's ambient ETH balance as unreserved. State transitions zero or decrement the source liability before transfer and restore exactly on failed delivery.

A harvest transfers liability from newly minted quote claim backing into service-fee plus pot ledgers. A flush transfers pot and prior-carry liability into tip, successful plugin value, creator-plugin entitlement, redirects, or new carry. Direct creator and global protocol claims operate only on their ledgers and use `_ensureEth` without changing pot or carry accounting.
...
The immediate `msg.sender` receives the tip. The payout path iterates the plan's at-most-eight set bits in ascending order. For each entry, `attempted = pluginShare_i + previousCarry_i`; carry is cleared before the external call. Success leaves it clear. Revert, invalid return behavior, or stipend exhaustion restores the full attempted value as carry. A suspended or code-invalid entry is not called: its attempted amount is credited to creator entitlement and its carry is permanently cleared. A zero attempted amount is skipped.

After plugin iteration, `creatorNew` plus all redirects is credited—not pushed—to `_creatorPluginClaimable[poolId]`. A carry-only flush iterates `_carryBitmap` with no redemption, allocation, or tip. Empty pot plus empty bitmap returns before any unlock or transfer.
...
**Decision:** Add a protocol-global payout-delivery transient lock in addition to existing pool-scoped locks. Every flush, direct/global claim, fee collection, graduation, governance execution path on the hook, and delegated payout entry checks it. While held, no pool can start another protected operation. Hook callbacks caused by a reference plugin's PoolManager interaction return without deployment, harvest, graduation, fee collection, or payout side effects.
...
**Decision:** The creator destination is not a selectable registry plugin. Flush credits `_creatorPluginClaimable[poolId]` inside hook storage. `claimCreatorPlugin(poolId)` verifies current RevenueNFT ownership, invokes the same flush machinery first, then zeros and transfers the complete resulting creator-plugin balance. Because the holder's call is the immediate flush call, any new-pot tip is paid back to that holder. If the final ETH transfer fails, the balance is restored and remains claimable.
...
Fee collection retains zero-delta `modifyLiquidity` but routes quote to direct creator plus global protocol and token to milestone inventory plus burn. When ladder capacity cannot use future inventory, all token fees burn. The graduation path alone may add full-range liquidity; its immutable 40/55/5 split remains separate from `EconomicConfig`. Source-level lock checking is extended to reject both negative and post-graduation positive changes involving `FULL_RANGE_SALT` outside the single graduation seed site.
...
- Unit suites cover registry immutability, plan validation and identity, typed governance, global economics, pot funding, exact flush math, suspension redirects, carry retry, gas exhaustion, same/cross-pool reentry, creator-plugin NFT semantics, buyback, helper composition, static fees, immediate dev buy, global protocol claiming, token burning, and fixed locked liquidity.
...
- **Gas-exhausting plugin prevents final accounting** → Enforce immutable per-entry limits, compute an outer reserve for remaining iterations/finalization, cap selected entries at eight, and adversarially test EIP-150 boundaries.
...
- **Claim or plugin transfer spends reserved ETH** → Maintain explicit liability totals and assert raw ETH plus quote claims cover direct, protocol, pot, carry, and creator-plugin balances after every handler action.
...
4. Implement `MilestonePayoutPaths`, dedicated pot redemption, explicit liability accounting, global transient delivery guard, carry, creator-plugin claiming, and callback suppression; extend all delegatecall guard/layout checks.
...
None. Contract names may change during implementation, but the contract split, custody transitions, governance authority, registry identity rules, and test obligations are fixed by this design.