...
contract ClaimsTest is HarnessLaunchpadTest {
    ...
    // (with the ledger's derived guards: credits sum, a zero accrual is a no-op, and the two
    // parties' balances are independent and per-pool)
    ...
    function test_accrualsFromAllSourcesAggregate() public {
        ...
        harness.accrueCreatorFrom(POOL_A, 2 ether, MilestoneBase.AccrualSource.MILESTONE_HARVEST);
        harness.accrueCreatorFrom(POOL_A, 3 ether, MilestoneBase.AccrualSource.MILESTONE_HARVEST);
    ...
    // --- The admin power reaches nothing else: derived, no scenario of its own ---
